    ...
    <div className="relative group">
      ...
      <div className="relative px-3 sm:px-4 py-2 rounded-full bg-white/80 backdrop-blur-xl border border-blue-200/50 shadow-lg">
        <span className="bg-gradient-to-r from-[#6366f1] to-[#a855f7] text-transparent bg-clip-text sm:text-sm text-[0.7rem] font-medium flex items-center">
          <Sparkles className="sm:w-4 sm:h-4 w-3 h-3 mr-2 text-blue-500" />
  ...
  <div className="space-y-2" data-aos="fade-up" data-aos-delay="600">
    <h1 className="text-5xl sm:text-6xl md:text-6xl lg:text-6xl xl:text-7xl font-bold tracking-tight">
      <span className="relative inline-block">
        ...
        <span className="relative bg-gradient-to-r from-gray-800 via-blue-600 to-purple-600 bg-clip-text text-transparent">
          Gia Sư
        </span>
      </span>
      <br />
...
const ExperienceCard = memo(({ icon: Icon, title, value, description }) => (
  <div className="px-4 py-3 rounded-xl bg-white/80 backdrop-blur-sm border border-blue-200/50 text-sm text-gray-700 hover:bg-white/90 transition-colors shadow-lg">
...
const CTAButton = memo(({ href, text, icon: Icon }) => (
  <a href={href}>
    <button className="group relative w-[160px]">
      ...
      <div className="relative h-11 bg-white/90 backdrop-blur-xl rounded-lg border border-blue-200/50 leading-none overflow-hidden shadow-lg">
        <div className="absolute inset-0 scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-500 bg-gradient-to-r from-[#4f52c9]/20 to-[#8644c5]/20"></div>
        <span className="absolute inset-0 flex items-center justify-center gap-2 text-sm group-hover:gap-3 transition-all duration-300">
          <span className="bg-gradient-to-r from-gray-700 to-gray-900 bg-clip-text text-transparent font-medium z-10">
            {text}
          </span>
          <Icon className={`w-4 h-4 text-gray-600 ${text === 'Liên Hệ' ? 'group-hover:translate-x-1' : 'group-hover:rotate-45'} transform transition-all duration-300 z-10`} />
        </span>
      </div>
    </button>
  </a>
...
const SocialLink = memo(({ icon: Icon, link }) => (
  <a href={link} target="_blank" rel="noopener noreferrer">
    <button className="group relative p-3">
      ...
      <div className="relative rounded-xl bg-white/80 backdrop-blur-xl p-2 flex items-center justify-center border border-blue-200/50 group-hover:border-blue-300/70 transition-all duration-300 shadow-lg">
